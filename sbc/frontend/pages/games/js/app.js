...
const GAMES = [
  { id: "tictactoe", label: "Tres en Raya", icon: "⭕", mount: mountTicTacToe },
...
export function mountGameHub(container, exitToHome) {
  ...
  input.on(InputAction.RIGHT, () => { selected = Math.min(selected + 1, cards.length - 1); render(); }, "MENU");
  input.on(InputAction.LEFT, () => { selected = Math.max(selected - 1, 0); render(); }, "MENU");
  input.on(InputAction.DOWN, () => { selected = Math.min(selected + 1, cards.length - 1); render(); }, "MENU");
  input.on(InputAction.UP, () => { selected = Math.max(selected - 1, 0); render(); }, "MENU");
